...
describe('Test the operations of the server', function() : void{
  ...
  it('Case 3', function () : void{
    ...
    expect(cmtResp.message).to.equal("Successfully added the commit as the head of commit");
    expect("Success").to.equal(cmtResp.className);
  });
  ...
  it('Case 1', function () : void {
    ...
    // resend
    var cmtResp : ResponseDT<IResponse> = new CommitOperations(cmt).processCommit(CommitCache, listOfCommits);
    expect(cmtResp.message).to.equal("This commit is already the head of the commit");
    expect(cmtResp.className).to.equal("Success");
  });
